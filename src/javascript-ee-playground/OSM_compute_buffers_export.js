...
var topo_par = rivers_lines_osm_segments.map(function(f) {
  var bufferMax = 150
...
rivers_lines_osm_segments = topo_segments

//Map.addLayer(rivers_lines_osm_segments, {color:'ff0000'}, 'osm, filtered')
...
function show() {
  ...
  Map.addLayer(rivers_lines_osm_segments, {palette:['ff0000']}, 'osm')
...
function performBufferOverlayAnalysis(analyzeDrainageNetwork, useFlowAccumulation) {
  ...
  print(computeBufferScores(ee.Feature(rivers_lines_osm_segments.first())))


  //var river_with_buffers = ee.FeatureCollection(rivers_lines_osm_segments.toList(1000, 0))
  //  .map(computeBufferScores)

  //print(river_with_buffers.filter(ee.Filter.neq('N_water_buffer_150', 0)).first())

  //var river_with_buffers = ee.FeatureCollection(rivers_lines_osm_segments)
  //    .map(computeBufferScores)
  ...
  if(!analyzeDrainageNetwork) {
    for(var i = 0; i < 7; i++) { 
      var river_with_buffers = ee.FeatureCollection(rivers_lines_osm_segments.toList(5000, i*5000))
        .map(computeBufferScores)
...
var analyzeDrainageNetwork = true
var useFlowAccumulation = true
