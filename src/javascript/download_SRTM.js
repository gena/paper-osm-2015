...
function processTile(featureId) {
  ...
  */
  ...
  var detectWater = function(water_index, water_index_name, visible, show_charts) {
    ...
    if(classifyManual) {
      var classified = classify(imageClassify.mask(water), trainedClassifier, classifierBandNames)
...
*/
    ...
    */
    ...
    if(water_index_name == 'mndwi') { // download just once, TODO: move to arguments
      var im = image.visualize({min:0.05, max:[max_I, max_I, max_I + 0.1], gamma:1.5});
      
      var name = pad(featureId, 5) + '_swir1_nir_green_' + percentile
      var url = im.clip(boundsAoi).getDownloadURL({
        name: name,
        format: 'png',
        crs: crs,
        //crs_transform: JSON.stringify(crsTransformSetStep(crs_transform[0] / 2, crs_transform)),
      ...
      if(!boundsIsEmpty) {
        var combined = ee.ImageCollection.fromImages([
            im,
            waterEdge.visualize({palette:['aaaaff']})
        ]).mosaic();
  
        var name = pad(featureId, 5) + '_swir1_nir_green_combined' + percentile
        var url = combined.clip(boundsAoi).getDownloadURL({
          name: name,
          format: 'jpg',
          crs: crs,
          crs_transform: JSON.stringify(crsTransformSetStep(crs_transform[0] / 2, crs_transform)),
...
Map.addLayer(rivers_lines_osm_image, {palette:['c60000'], opacity: 0.7}, 'rivers_lines', false);
